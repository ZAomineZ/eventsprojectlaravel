...
export default class UpdateEvent extends PureComponent {
    constructor(props) {
        super(props);
        this.state = {
            title: '',
    ...
    handleRemoveImage(file)
    {
        ...
        if (user) {
            ...
            formData.append('slugEvent', slug);
    ...
    componentDidMount() {
        ...
        if (isString(slug)) {
            ...
            axios.get('/api/event/update/' + slug, {
                headers: headers
            }).then(response => {
                if (response.data.success === true) {
                    let event = response.data.data.event;
                    let positionPlace = response.data.data.positionPlace.position;
                    let viewport = JSON.parse(response.data.data.positionPlace.viewport);
                    ...
                    this.setState({
                        title: event.title,
                        date: new Date(event.date_event),
                        place: event.place,
                        user_max: event.users_max,
        ...
        if (this.validForm(errors)) {
            ...
            axios.post('/api/event/update/' + slug, {
                ...
                if (response.data.success === true) {
                    ...
                    NotifAppMessage('success', response.data.data.message, 'Well Done !!!');
                    this.props.history.push('/event/show/' + slug);
                } else {
                    NotifAppMessage('danger', response.data.message, 'Warning !!!');
                }
            })
    ...
    render() {
        const {viewport, searchResultCity, errors} = this.state;
            ...
            <HeaderHome>
                <main id="main-container">
                    <div className="content">
                        <h2 className='content-heading'>Update your project</h2>
                        <div className="block">
                                        ...
                                        <div className="block-content block-content-full">
                                            <form action="#" method="post" encType="multipart/form-data"
                                                ...
                                                <div className="form-group row">
                                                    ...
                                                    <div className="col-lg-8">
                                                        <select className="form-control" name="user_max" id="users"
                                                                onChange={this.handleChange}>
                                                            <option value="2" selected={this.state.user_max === 2}>2
                                                            </option>
                                                            <option value="4" selected={this.state.user_max === 4}>4
                                                            </option>
                                                            <option value="6" selected={this.state.user_max === 6}>6
                                                            </option>
                                                            <option value="8" selected={this.state.user_max === 8}>8
                                                            </option>
                                                            <option value="12"
                                                                    selected={this.state.user_max === 12}>12
                                                            </option>
                                                            <option value="16"
                                                                    selected={this.state.user_max === 16}>16
                                                            </option>
                                                        </select>
                                                    </div>
                                                </div>
                                                <div className="form-group row">
                                                    ...
                                                    <div className="col-lg-8">
                                                        <select className="form-control" name="category_id" id="users"
                                                                onChange={this.handleChange}>
                                                            <option value='1'
                                                                    selected={this.state.category_id === 1}>Sport
                                                            </option>
                                                            <option value='2'
                                                                    selected={this.state.category_id === 2}>Jeux Videos
                                                            </option>
                                                            <option value='3'
                                                                    selected={this.state.category_id === 3}>Cinema
                                                            </option>
                                                            <option value='4'
                                                                    selected={this.state.category_id === 4}>Travail
                                                            </option>
                                                            <option value='5'
                                                                    selected={this.state.category_id === 5}>Loisir
                                                            </option>
                                                        </select>
                                                    </div>
                                                </div>
                                                <div className="form-group row">
                                                    ...
                                                    <div className="col-lg-8">
                                                        <select className="form-control" name="type_event"
                                                                onChange={this.handleChange}>
                                                            <option value='public'
                                                                    selected={this.state.type_event === 'public'}>Public
                                                            </option>
                                                            <option value='private'
                                                                    selected={this.state.type_event === 'private'}>Private
                                                            </option>
                                                        </select>
                                                    </div>
                                                </div>
                                                ...
                                                <textarea name="content"
                                                          ...
                                                          rows='8' value={this.state.content}/>
                                                {
                                                ...
                                                <h2 className="content-heading">Vos images actuels</h2>
                                                <div className="row items-push">
                                                    {
                                                        this.state.images.length > 0 ?
                                                            this.state.images.map(d => {
                                                                return (
                                                ...
                                                <div className="">
                                                    {
                                                        user ?
                                                            <Upload
                                                                action={'/api/event/updated/' + slug + '/' + user.user.id}
                                                ...
                                                <button className='btn btn-primary mt-5'>Update Event</button>
                                            </form>
                                        </div>
                                    </div>
                                </div>
